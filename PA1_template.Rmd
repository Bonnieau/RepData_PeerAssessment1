...
```{r fig.width=6,fig.height=6}
    # Make a histogram of the total number of steps taken each day
    barplot(step.day$steps,names.arg=step.day$date,main="Histogram of Total Steps Taken Each Day",xlab="date",ylab="total steps",col="red")
```
...
```
#### The mean total number of steps taken each day is `r format(step.mean,scientific=F)`
#### The median total number of steps taken each day is `r format(step.median,scientific=F)`
...
```{r fig.width=6,fig.height=6}
    ...
    plot(interval.mean,type="l",main="Average Daily Activity Pattern",
         xlab="5 minutes intervals",ylab="average steps taken over all days",col="blue")
```
...
```
#### The 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps (`r interval.max$steps`) is `r interval.max$interval`
...
```
#### The total number of rows with missing value is `r count.na`
...
```{r}
    ...
    newds<-myds
    # Replacing the missing steps with the mean of the 5 minutes interval mean across all day
...
#### The new dataset has total row number `r nrow(newds)` , with columns contain `r names(newds)` 
#### The original dataset has total row number `r nrow(myds)` with columns contain `r names(myds)`
#### The missing value counts in new dataset is `r sum(is.na(newds))` while the original missing data count is `r sum(is.na(myds))`
...
```{r fig.width=6, fig.height=6}
    # Make a histogram of the total number of steps taken each day
    barplot(newstep.day$steps,names.arg=newstep.day$date,main="Histogram of Total Steps Without Missing Values",xlab="date",ylab="total steps",col="green")
```
...
```
#### The mean total number odf steps taken each day is `r format(newstep.mean,scientific=F)`
#### The median total number odf steps taken each day is `r format(newstep.median,scientific=F)` 
...
#### from original `r format(step.median,scientific=F)` to new median value `r format(newstep.median,scientific=F)`.
...
```{r}
    ...
    newds$day<-as.factor(sapply(newds$date,function(x) ifelse(weekdays(x) %in% weekend,"weekend","weekday")))
```
...
```{r fig.width=6,fig.height=6}
    ...
    library(lattice)
    xyplot(steps~interval | day,data=newstep.average,layout=c(1,2),type="l",xlab="5-minute intervals over day", ylab="number of steps",main="Activity Patterns on Weekends and Weekdays")
